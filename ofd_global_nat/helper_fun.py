...
def get_cma_packages_list(cp):
    ...
    """
    data1 = {}
    data = {"limit": 500, "offset": 0, "details-level": "standard"}
    data1["data"] = data
    ...
    if data.status_code == 200:
        ...
        for pkg_list in cp_data["packages"]:
            ...
            table_data.append(get_nat_rulebase(cp, pkg_list["name"]))
        return table_data


def parse_data(cp, pkg, cp_data):
        ...
        if "rulebase" in rule_dict:
                ...
                if "static" in rule_dict1["method"]:
                    ...
                    if "Any" not in uid_map.get(rule_dict1["original-source"]) or "Original" not in uid_map.get(
                        ...
                        if rule_dict1["auto-generated"] == False:
                            table_data.append(nat_var(rule_dict1, uid_map, pkg))
                        else:
            ...
            if "static" in rule_dict["method"]:
                if "Any" not in uid_map.get(rule_dict["original-source"]):
                    table_data.append(nat_var(rule_dict, uid_map, pkg))
            else:
                table_data.append(nat_var(rule_dict, uid_map, pkg))
    return table_data


def nat_var(rule_dict1, uid_map, pkg):
    """Variable function.
...
def get_nat_rulebase(cp, pkg):
    ...
    while j <= total_rule:
        ...
        if total_rule == 0:
            ...
            table_data.extend(parse_data(cp, pkg, cp_data))
            j = j + 500
